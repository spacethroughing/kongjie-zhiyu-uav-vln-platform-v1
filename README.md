...
- 每完成或阻塞一个航点，确定性前沿评分器都会在有界候选窗口内重新计算端点覆盖率、整段 LiDAR 覆盖率、实际航迹重访率、连接距离和航向变化，优先选择信息增益更高且少走旧路的安全前沿。
- 局部绕障把实际航迹重访率加入候选代价，普通水平绕障限制在 90° 内；无法安全脱困时放弃当前前沿并重新规划，而不是执行 110° 反向回退。
...
拓扑规划输入包含最近拓扑节点/边、地图统计和连续覆盖路线前方最多 8 个候选航点。候选携带 `coverage_ratio`、`segment_coverage_ratio`、`trajectory_revisit_ratio`、`explored`、`novelty_m`、相邻关系和目标语义线索距离。

v1 为避免模型将远距离扫描带重新排序造成跨图跳转，拓扑 VLM 只能在连续局部前视窗口内建议目标。确定性前沿评分器先生成安全的局部重排，VLM 只有在建议连接段没有增加明显重访或大角度反转时才能提升其中一个候选；其余航点保留为割草路线回退。飞行每完成或阻塞一个航点都会再次执行确定性前沿评分，但不会每次调用拓扑 VLM。详见[已知限制](#v1-已知限制与后续方向)。
...
当前版本已经分别维护 LiDAR 已观察栅格和实际飞行轨迹栅格，并对完整候选连接段计算覆盖率与轨迹重访率。该代价用于前沿重排和局部绕障，但仍属于有界软惩罚：当未探索前沿只能经旧区域到达、直接连接被障碍阻塞，或回退覆盖路线必须经过旧航带时，仍可能出现必要重访。当前还没有占据栅格 A* 和带时间衰减的冷却层。详见下一节。
...
1. **拓扑 VLM 不是持续全局规划器。**当前仅在搜索开始和人工视点改变后低频调用；每个航点后的重排由确定性前沿评分器完成。
2. **尚未实现占据栅格 A* / Dijkstra 连接规划。**当前在有界候选窗口内选择地理围栏允许的直接连接，再由滚动 LiDAR 局部规划器绕障；复杂障碍可能迫使系统跳过前沿。
3. **轨迹重访是软惩罚且没有时间冷却衰减。**规划器会显著惩罚旧航迹，但不会永久禁止为了安全或可达性而穿越旧区域。
...
- 将当前 LiDAR 探索层、占据层和实际轨迹层进一步统一为可搜索的全局代价地图；
- 在地图信息增益显著变化时低频触发拓扑 VLM，而不是只在搜索开始或人工视点改变后调用；
- 用 A* / Dijkstra / kinodynamic planner 生成受安全约束的连接路径；
- 为刚访问网格增加带时间衰减的冷却层，并区分必要重访与无收益重访；
